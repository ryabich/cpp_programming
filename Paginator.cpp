...
class Paginator {
public:
	Paginator(Iterator _begin, Iterator _end, size_t _size) {
		Iterator begin = _begin;
		size_t allsize = _end - begin;
		while (allsize > _size) {
			auto it = next(begin, _size);
			pages.push_back({begin, it});
			begin = it;
			allsize -= _size;
		}
		if (allsize > 0)
			pages.push_back({begin, _end});
	}
